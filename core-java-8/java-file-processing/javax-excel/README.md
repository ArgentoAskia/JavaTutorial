...
在`Java`中，常用的处理`Excel`的库有`4`个，我们可以总结为一个核心三大组件：一个核心即**Apache POI**，作为最老牌、功能最强大、`API`最丰富的处理库，提供了非常底层的操作`API`，而三大组件即为来自阿里巴巴的`EasyExcel`（现`FastExcel`，基于`POI`封装），轻量级`POI`封装`EasyPOI`，流式解析`OpenXlsx`。

在具体介绍`Excel`库之前，我们需要了解`Excel`的基本元素，这对我们进行`Excel`操作非常有效，熟悉`office`操作的朋友基本都知道，一般一个`Excel`表格的核心组成元素可以分为四个，即：工作簿（`xls`、`xlsx`文件本身，也叫`workbook`）、工作表（`sheet`，位于表格的下方![image-20250912004901757](README/image-20250912004901757.png))、行和列、单元格。这四个核心元素将贯穿整个`Excel`文件，基本所有的操作都基于这四个元素。
