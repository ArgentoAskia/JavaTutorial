...
//
    ...
    protected void initMenu(){
        ...
        // TODO: 2024/2/1 并非不生效，而是菜单不能设置快捷键！
        menuBar = new MenuBar();
    ...
    private Menu menu1;
    private MenuItem menu1MenuItem1;
    private MenuItem menu1MenuItem2;
    private CheckboxMenuItem menu1CheckboxMenuItem1;
    private CheckboxMenuItem menu1CheckboxMenuItem2;
    private Menu menu1SubMenu1;
    private MenuItem menu1SubMenu1MenuItem1;
    private MenuItem menu1SubMenu1MenuItem2;
    private Menu menu2;
    private MenuItem menu2MenuItem1;
    private CheckboxMenuItem menu2CheckboxMenuItem1;

    // 对话框
    // 显示对话框的Menu
    private Menu dialogsMenu;
    // common dialog
    private Menu simpleDialogMenu;
    private MenuItem showMessageBoxMenuItem;
    private MenuItem showInputBoxMenuItem;
    private Dialog messageDialog;
    private Dialog inputDialog;

    // file dialog
    private Menu fileDialogMenu;
    ...
    private void initMenuWidgets(){
        menuComponent = new Menu("菜单组件", true);
        menuItem1 = new MenuItem("菜单项按钮1", new MenuShortcut(KeyEvent.VK_A));
        menuItem2 = new MenuItem("菜单项按钮2", new MenuShortcut(KeyEvent.VK_B));
        checkboxMenuItem1 = new CheckboxMenuItem("菜单项选择框1");
        // 参数2：是否选上！
        checkboxMenuItem2 = new CheckboxMenuItem("菜单项选择框2", true);
        checkboxMenuItem2.setShortcut(new MenuShortcut(KeyEvent.VK_E));


        menu1 = new Menu("菜单1");
        menu1MenuItem1 = new MenuItem("菜单1菜单项按钮1", new MenuShortcut(KeyEvent.VK_C, true));
        menu1MenuItem2 = new MenuItem("菜单1菜单项按钮2", new MenuShortcut(KeyEvent.VK_D, true));
        menu1CheckboxMenuItem1 = new CheckboxMenuItem("菜单1菜单项选择框1", true);
        menu1CheckboxMenuItem2 = new CheckboxMenuItem("菜单1菜单项选择框2");
        menu1SubMenu1 = new Menu(" 菜单1子菜单");
        menu1SubMenu1MenuItem1 = new MenuItem("菜单1子菜单菜单项1");
        menu1SubMenu1MenuItem2 = new MenuItem("菜单1子菜单菜单项2");
        menu1SubMenu1.add(menu1SubMenu1MenuItem1);
        menu1SubMenu1.add(menu1SubMenu1MenuItem2);
        menu1.add(menu1MenuItem1);
        menu1.add(menu1MenuItem2);
        menu1.add(menu1CheckboxMenuItem1);
        menu1.add(menu1CheckboxMenuItem2);
        menu1.add(menu1SubMenu1);

        menu2 = new Menu("菜单2");
        menu2MenuItem1 = new MenuItem("菜单2菜单项按钮");
        menu2CheckboxMenuItem1 = new CheckboxMenuItem("菜单2菜单项选择框");
        menu2.add(menu2MenuItem1);
        menu2.add(menu2CheckboxMenuItem1);

        menuComponent.add(menuItem1);
        menuComponent.add(menuItem2);
        // 添加 - 分割符号
        menuComponent.add("-");
        menuComponent.add(checkboxMenuItem1);
        menuComponent.add(checkboxMenuItem2);
        // 另外一种方法
        menuComponent.addSeparator();
    ...
    private void initMenuItemsListener(){
        ActionListener actionListener = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String message = "Hello, you press the ";
                // get Key
                final int modifiers = e.getModifiers();
                final Object source = e.getSource();
                if (!(source instanceof CheckboxMenuItem)){
                    MenuItem menuItem = (MenuItem) source;
                    message = message + "menuItem:" + menuItem.getLabel();
                }
                if (modifiers != 0){
                    message = message + " with Shortcut";
                }else{
                    message = message + " with mouse";
                }
                JOptionPane.showMessageDialog(AWTComponents.this, message);
            }
        };
        menuItem1.addActionListener(actionListener);
        menuItem2.addActionListener(actionListener);
        menu1MenuItem1.addActionListener(actionListener);
        menu1MenuItem2.addActionListener(actionListener);
        menu1SubMenu1MenuItem1.addActionListener(actionListener);
        menu1SubMenu1MenuItem2.addActionListener(actionListener);
        menu2MenuItem1.addActionListener(actionListener);
    }

    // tear-off Menu: 分离式菜单
    // Menu 无法设置快捷键,而MenuItem、CheckboxMenuItem都可以设置快捷键
    private void initDialogMenu(){
        dialogsMenu = new Menu();
        dialogsMenu.setLabel("对话框菜单");
        dialogsMenu.setShortcut(new MenuShortcut(KeyEvent.VK_D + KeyEvent.VK_ALT));

        simpleDialogMenu = new Menu("普通对话框", true);
        showMessageBoxMenuItem = new MenuItem("信息框...");
        showInputBoxMenuItem = new MenuItem("输入框...");
        simpleDialogMenu.add(showMessageBoxMenuItem);
        simpleDialogMenu.add(showInputBoxMenuItem);

        fileDialogMenu = new Menu("文件对话框");
        ...
        fileDialogMenu.add(showOpenFileMenuItem);
        fileDialogMenu.add(showSaveFileMenuItem);

        // 对话框组件添加
        dialogsMenu.add(simpleDialogMenu);
        dialogsMenu.add(fileDialogMenu);

        showOpenFileMenuItem.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                JOptionPane.showMessageDialog(AWTComponents.this, "Hello");
            }
        });
    }
    ...
    private void initDialogMenuItemEvent(){
        showMessageBoxMenuItem.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
        ...
        showInputBoxMenuItem.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
        ...
        showSaveFileMenuItem.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
        ...
        saveFileDialog.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosed(WindowEvent e) {
                final String file = saveFileDialog.getDirectory();
                System.out.println(file);
            }
        });
    }

    protected void initMenuAndDialog() {

        menuBar = new MenuBar();

        initMenuWidgets();
        initMenuItemsListener();

        initDialogMenu();
        initDialogs();
        initDialogMenuItemEvent();

        menuBar.add(menuComponent);
        menuBar.add(dialogsMenu);
        setMenuBar(menuBar);
    }
    ...
    public AWTComponents(){
        // 初始化Panel，即组件Panel
        layoutPanel();
        // 初始化组件布局，即将组件放入组件Panel
        layoutComponents();
        // 初始化菜单
//        initMenu();
        // 初始化菜单组件和对话框
        initMenuAndDialog();
        // 初始化外部Panel（外部Panel = 组件Panel + 说明文字）
        // 初始化Frame大小设置
        initWindows();
        // 运行动画
        runAnimation();
