...
@SupportedAnnotationTypes("cn.argento.askia.compiler.annotation.ToString")
@SupportedSourceVersion(SourceVersion.RELEASE_8)
public class DynamicCompilerTaskToStringAnnotationProcessor extends AbstractProcessor {
...
//            final Annotation[] annotationsByType = typeElement.getAnnotationsByType();
    ...
    private void toStringAnnotationProcess(RoundEnvironment currentRoundEnv, TypeElement typeElement) throws IOException {
        // 注解形式的情况下，下面的代码会生成在D:\OpenSourceProject\JavaProject下面，因此不能直接用！
        // 解决方法要么我们自己创建一个JavaFileObject对象，这个比较麻烦
        // 要么指定编译参数-s（比较简单！）
    ...
    private void printlnReference(PrintWriter out){
        ...
        out.println("// Automatically generated by "
                + "cn.argento.askia.compiler.annotation.DynamicCompilerTaskToStringAnnotationProcessor");
        out.println("// Thanks for core java 11, it is a excellent book to read!!!");
        out.println();
        out.println("package cn.argento.askia.compiler.annotation;");
