...
public class InetAddressDemo {
    ...
    private static void inet6AddressDemo() throws UnknownHostException {
    }

    private static void inetAddressDemo() throws IOException {
        // 1. 使用静态方法，提供Host和IpAddress来创建一个InetAddress对象
        // 其中IpAddress使用字节表示
        byte[] addressBytes = {(byte)223, (byte)220, 0, 101};
        InetAddress address = InetAddress.getByAddress(addressBytes);
        // 或者提供Host和IP地址来进行创建，注意这种创建方式不会进行DNS解析！因此IP和主机名可能是Faker的！
        ...
        System.out.println("www.argentoaskia.cn域名解析出来的结果：" + host2);
        System.out.println();
